...
#        print status.text 
	if not status.retweeted and 'RT @' not in status.text:
	     ...
	     if len(status.text.split()) < 8:
		if 'http' not in status.text and status.user.location != 'none':
		    newStatus = {}
		    #g = geocoder.google(c)
...
#		    print "********  " + status.text + "  ********"
		    ...
		    newStatus["location"] = status.user.location 
...
#		    with open(j, 'w') as f:
#			j = json.dump(data, f)	
           	    r.db("projec").table("tweetsTrialTwo").insert(j).run()    
	return True
...
#    r.connect("localhost", 33408).repl()
    stream = Stream(auth, l)

    #This line filter Twitter Streams to capture data by the keywords: 'python', 'javascript', 'ruby'
    stream.filter(track=['earth quake', 'earth-quake', 'earthquake', 'seismic activity', 'seismic'])
